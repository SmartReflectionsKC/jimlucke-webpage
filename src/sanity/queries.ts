 ...
 */
export const FIELD_NOTES_LIST_QUERY = `*[_type == "fieldNote" && !(_id in path("*.**")) && !(_id in path("drafts.**"))] | order(publicationDate desc) {
  _id,
  title,
  "slug": slug.current,
 ...
 */
export const FIELD_NOTE_DETAIL_QUERY = `*[_type == "fieldNote" && slug.current == $slug && !(_id in path("*.**")) && !(_id in path("drafts.**"))][0] {
  _id,
  title,
  "slug": slug.current,
  "date": publicationDate,
 ...
 */
export const GALLERIES_QUERY = `*[_type == "gallery" && !(_id in path("*.**")) && !(_id in path("drafts.**"))] | order(displayOrder asc) {
  _id,
  "id": slug.current,
  title,
  description,
